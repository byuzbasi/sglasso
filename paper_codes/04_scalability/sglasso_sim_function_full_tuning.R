...
run_scalability_experiment <- function(p_grid = c(3000, 10000, 20000, 50000),
                                       J_grid = c(1000, 2000, 5000, 10000),
                                       pj = 3,
                                       n_train = 100,
                                       n_val = 0,
                                       n_test = 0,
  ...
  run_one_runtime <- function(axis_name, p_value, J_value, rep_id) {
    ...
    strong_J <- if (is.null(active_groups) || is.na(active_groups)) {
      max(1, round(strong_frac * J_value))
    } else {
      max(1, min(as.integer(active_groups), J_value))
    }
    ...
    data.frame(
      ...
      rep_id = rep_id,
      strong_J = strong_J,
      active_group_fraction = strong_J / J_value,
      signal_pattern = signal_pattern,
      lambda_min_ratio = lambda_min_ratio,
      standardize_external = isTRUE(standardize),
